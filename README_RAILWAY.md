...
| `docker/start-all.sh` | 서비스 7개와 콘솔을 띄우고, 죽으면 되살리는 감시 루프를 돈다 |
...
한 컨테이너 안에서 프로세스 8개가 돌기 때문에, 그중 하나만 죽어도
Railway는 알아채지 못한다. `restartPolicyType`은 컨테이너의 **주 프로세스**만 보고,
헬스체크도 콘솔(:8080) 하나만 본다. 그래서 `docker/start-all.sh`가 직접 감시한다.

- 10초마다(`WATCH_INTERVAL`) 8001~8007과 콘솔 포트에 **직접 붙어 본다**.
  PID가 아니라 포트를 보는 이유는, 죽은 자식이 좀비로 남으면 `kill -0`이 참을 내놓기 때문이다.
- 서비스가 응답하지 않으면 그 서비스만 다시 띄운다. 나머지는 건드리지 않는다.
  갓 띄운 것은 `WATCH_GRACE`(기본 25초) 동안 죽었다고 보지 않는다.
- **콘솔이 죽으면** 되살리지 않고 컨테이너를 내린다(`exit 1`).
  콘솔은 주 프로세스라, 내리면 Railway가 컨테이너째 새로 띄우는 쪽이 깨끗하다.

로그에 이렇게 남는다.

```
[watchdog] 04-scheduler (:8004) 응답 없음 — 재시작 #1
```

같은 일을 로컬에서는 `watchdog.ps1`이 한다. `start_all.ps1`이 마지막 단계에서
자동으로 띄우고, `stop_all.ps1`이 (서비스를 내리기 전에 먼저) 끈다.
자세한 건 [README_사용자가이드.md](README_사용자가이드.md) 참고.
