...
# 면접일 · 시간대
#
# 날 이름은 요일이 아니라 **면접 며칟날인지** 를 가리킨다. 우리는 무슨 요일에
# 보는지를 셈에 넣지 않는다 — 담당자에게 가능 요일을 묻지 않고, 배치 규칙 어디도
# 월요일과 목요일을 다르게 보지 않는다. 그런데 이름이 '월 · 화' 이면 사람은
# 달력을 떠올려 "우리 팀은 왜 월요일이 없나" 를 묻게 된다. 실제 달력에 언제
# 붙일지는 인사가 나중에 정할 일이고, 여기서는 몇째 날인지만 센다.
#
...
#: 예전 표기(요일) → 지금 표기(일차). 저장해 둔 회차 자료에 남아 있다.
LEGACY_DAYS = dict(zip(["월", "화", "수", "목", "금"], DAYS))


def day_name(day) -> str:
    """요일로 저장해 둔 날 이름을 지금 이름(n일차)으로 — 모르는 값은 그대로."""
    text = str(day or "").strip()
    return LEGACY_DAYS.get(text, text)
...
def normalize_availability(availability, timing=None) -> dict[str, list[str]]:
    ...
    ② **날을 지운다.** 우리 모델에 담당자 가능 요일은 없다. 어느 날 칸에 적어
       냈든 고른 칸을 모아 모든 날에 똑같이 편다. 예전에는 요일별로 받아 두고
       배치는 그 요일을 그대로 믿었는데, 정작 화면 어디에서도 요일을 고르게
       하지 않아 자료에 남은 요일이 그 사람의 뜻과 상관없이 자리를 막았다 —
...
def plan_team_days(sizes_by_team, days_per_team: int = DAYS_PER_TEAM) -> dict[str, list[str]]:
    ...
    예전에는 날을 요일로 부르면서 팀마다 '한산한 요일' 을 골라 나눠 줬다. 큰 팀이
