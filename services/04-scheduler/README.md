...
| 규칙 | 종류 | 점수 산식 |
|---|---|---|
| `rule1_grad_balance` | SOFT | 날별 대학원 비율이 `target ± tolerance`(기본 ±20%p) 안에 드는 날의 비율. `target`을 안 주면 **그 회차 명단의 실제 비율**이 목표다 — 재는 것은 "요일 분산"이지 3할이 아니라서, 대학원생이 1할인 회차에 3할을 들이대면 고르게 나눠도 0점이 나온다. 인사가 숫자를 못 박으면 그 값을 저장해 두고 검증할 때도 같은 값으로 잰다 |
