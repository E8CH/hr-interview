...
Stage 1 : 팀 × 면접일 배정 — 팀당 2일
Stage 1b: 일차별 학사/대학원 쿼터 (규칙1)
Stage 2 : 팀-면접일 그룹을 연속 시간대에 배치 (규칙3 세로 연속, 규칙4 첫 타임)

특성: 규칙 준수율은 높지만 팀당 수용량이 2일 × 8타임 = 16슬롯으로 제한되어
...
def run(
    ...
    *,
    ...
    # Stage 1 — 인사가 명단을 보낼 때 정해 둔 면접일이 있으면 그대로 쓴다
