...
면접위원에게 초대를 발송하고, 구조화 웹폼(요일×시간 슬롯 그리드)으로 가능 시간을 수집하며,
...
### 3.2 구조화 웹폼
`app/templates/form.html` — 요일(월~금) × 시간(09/10/11/14/15/16시) **30칸 토글 그리드**.
...
요일/시간 리터럴 검사, 중복 슬롯, `max_daily` 범위(1~6), 백업 연락처 형식을 추가로 본다.
