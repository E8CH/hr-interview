...
open_days)` 은 `open_days[team]`(그 팀 담당자가 한 명이라도 나올 수 있는 요일)
안에서 먼저 고르고, 그것만으로 자리가 모자랄 때만 남은 요일로 채운다. 인원 수만
보고 정하면 목 · 금만 되는 팀에 월 · 화 · 수를 줘, 부서 화면은 전원을 "빈 자리가
없습니다" 로 밀어 내고 자동배치는 아무에게나 한 칸씩 얹으며 최종 시간표는 그
자리를 죄다 옮긴다. `open_days` 를 안 주면 예전처럼 부하로만 고른다(옛 호출자
보호). 회귀 시험은 `services/04-scheduler/tests/test_team_days.py`.
...
가능 시간은 덩어리(앞타임 · 뒤타임 · 둘 다)로 받아 칸 목록으로 펼쳐 저장하고,
